...
export const getPost = asyncHandler(
    async (req: RequestWithUser, res: Response, next: NextFunction) => {
        ...
        if (!post) {
            return next(
                new ErrorResponse(`The requested post does not exist`, 401)
            );
        }
...
export const createPost = asyncHandler(
    async (req: RequestWithUser, res: Response, next: NextFunction) => {
        ...
        if (!title || !description) {
            return next(
                new ErrorResponse(`Please add a title, description`, 404)
            );
        }
...
// @route     GET /api/post/user/:username
// @access    Private
...
export const likePost = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
        ...
        if (!post) {
            return next(
                new ErrorResponse('The requested post does no exist', 401)
            );
        }
        //Loads like
